function playerBuilder(game) {
    ...
    function buildHero(x=400, y=500) {
        let hero;
        //  The hero!
        hero = game.add.sprite(x, y, 'ship');
        hero.health = 100;
        hero.anchor.setTo(0.5, 0.5);
        game.physics.enable(hero, Phaser.Physics.ARCADE);
        hero.body.maxVelocity.setTo(MAXSPEED, MAXSPEED);
        hero.body.drag.setTo(DRAG, DRAG);
        hero.weaponLevel = 1
        hero.events.onKilled.add(function () {
            shipTrail.kill();
        });
        hero.events.onRevived.add(function () {
            shipTrail.start(false, 5000, 10);
        });
        return hero;
    }
...
function create() {
    ...
    //  Add an emitter for the ship's trail
    shipTrail = b.buildShipTrail(game);
    hero2Trail = b.buildShipTrail(game);
...
function update() {
    ...
    //  Fire bullet
    if (hero2.alive && (fireButton.isDown || game.input.activePointer.isDown)) {
        fireBullet();
    }
